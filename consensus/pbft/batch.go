...
func newObcBatch(id uint64, config *viper.Viper, stack consensus.Stack) *obcBatch {
	...
	//王剑修改，最大允许N*10个通道，动态加节点机
	op.broadcaster = newBroadcaster(id, op.pbft.N*10, op.pbft.f, op.pbft.broadcastTimeout, stack)
...
func (op *obcBatch) processMessage(ocMsg *pb.Message, senderHandle *pb.PeerID) events.Event {
	...
	if ocMsg.Type == pb.Message_CHAIN_TRANSACTION {
		if len(ocMsg.Payload) > 100000 {
			logger.Info("Receive big transaction msg: %s size: %d", ocMsg.Type, len(ocMsg.Payload))
		}
