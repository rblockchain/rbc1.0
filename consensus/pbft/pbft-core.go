...
// sendCurrentViewEvent provides "keep-alive" null requests
type sendCurrentViewEvent struct{}
...
type pbftCore struct {
	...
	//stateTransferring bool               // Set when state transfer is executing 王剑屏弊，去掉此参数
	...
	oneSecondTimeout      time.Duration
...
func newPbftCore(id uint64, config *viper.Viper, consumer innerStack, etf events.TimerFactory) *pbftCore {
	...
	instance.oneSecondTimeout, err = time.ParseDuration("1s")
	...
	//根节点系统启动时发送一次SendCurrentView,王剑增加，同步可能的帐本
	instance.startSendCurrentTimer(instance.oneSecondTimeout, "start peer and send current view")
...
func (instance *pbftCore) ProcessEvent(e events.Event) events.Event {
	...
	case sendCurrentViewEvent:
		instance.sendCurrentView()
	case *pbftMessage:
		return pbftMessageEvent(*et)
	case pbftMessageEvent:
		msg := et

		logger.Debugf("Replica %d received incoming message from %v %v", instance.id, msg.sender, msg.msg)
	...
	case *CurrentView:
		return instance.recvCurrentView(et)
	case *ViewChange:
	...
	case stateUpdatedEvent:
		if instance.consumer.IsStateInProgress() {
			logger.Infof("Replica %d application skipInProgress is false6", instance.id)
		...
		logger.Infof("Replica %d application skipInProgress is false1", instance.id)
		instance.skipInProgress = false
	...
	case viewChangeQuorumEvent:
		//王剑增加，如果skipInProgess是true,则不进行同步
		if instance.skipInProgress {
			logger.Warningf("Replica %d received view change quorum, instance.skipInProgress is true", instance.id)

			return nil
		}
		//增加结束

		logger.Infof("Replica %d received view change quorum, processing new view", instance.id)
	...
	case viewChangeResendTimerEvent:
		if instance.activeView || instance.consumer.IsStateInProgress() {
			logger.Warningf("Replica %d had its view change resend timer expire but it's in an active view or IsStateInProgress , this is benign but may indicate a bug", instance.id)
			return nil
		}

		instance.view-- // sending the view change increments
...
func (instance *pbftCore) recvMsg(msg *Message, senderID uint64) (interface{}, error) {
	if reqBatch := msg.GetRequestBatch(); reqBatch != nil {
		...
		return chkpt, nil
	} else if vc := msg.GetCurrentView(); vc != nil {
...
func (instance *pbftCore) sendPrePrepare(reqBatch *RequestBatch, digest string) {
	...
	logger.Infof("Primary %d broadcasting pre-prepare begin for view=%d/seqNo=%d and digest %s", instance.id, instance.view, n, digest)
	instance.innerBroadcast(&Message{Payload: &Message_PrePrepare{PrePrepare: preprep}})
	logger.Infof("Primary %d broadcasting pre-prepare end for view=%d/seqNo=%d and digest %s", instance.id, instance.view, n, digest)
...
func (instance *pbftCore) recvPrePrepare(preprep *PrePrepare) error {
	logger.Infof("Replica %d received pre-prepare from replica %d for view=%d/seqNo=%d instance.h=%d instance.skipInProgress is %v ",
		instance.id, preprep.ReplicaId, preprep.View, preprep.SequenceNumber, instance.h, instance.skipInProgress)

	if !instance.activeView {
		logger.Infof("Replica %d ignoring pre-prepare as we are in a view change", instance.id)
		return nil
	}
...
func (instance *pbftCore) recvPrepare(prep *Prepare) error {
	...
	if !instance.inWV(prep.View, prep.SequenceNumber) {
		...
		//王剑增加，如果接收到的view比当前view大，发送当前view进行同步处理
		if instance.view < prep.View {
			instance.sendCurrentView()
		}
...
func (instance *pbftCore) stateTransfer(optional *stateUpdateTarget) {
	if !instance.skipInProgress {
		logger.Infof("Replica %d application skipInProgress is true2", instance.id)
		instance.skipInProgress = true
		instance.consumer.invalidateState()
	}
	logger.Infof("Replica %d application retryStateTransfer", instance.id)
...
func (instance *pbftCore) retryStateTransfer(optional *stateUpdateTarget) {
	if instance.currentExec != nil {
		logger.Infof("Replica %d is currently mid-execution, it must wait for the execution to complete before performing state transfer", instance.id)
		return
	}

	//if instance.stateTransferring {   //王剑修改
	if instance.consumer.IsStateInProgress() {
		logger.Infof("Replica %d is currently mid state transfer, it must wait for this state transfer to complete before initiating a new one", instance.id)
		return
	}
	...
	//王剑屏弊，去掉此参数,在同步处理时，skipInProgress设为未执行
	//instance.stateTransferring = true
	//logger.Infof("Replica %d application skipInProgress is false5", instance.id)
	//instance.skipInProgress = false

	logger.Infof("Replica %d is initiating state transfer to seqNo %d", instance.id, target.seqNo)
...
func (instance *pbftCore) execDoneSync() {
	if instance.currentExec != nil {
		...
		logger.Infof("Replica %d application skipInProgress is true3", instance.id)
		instance.skipInProgress = true
	}
	...
	if chkpt.SequenceNumber < H {
		...
		if len(instance.hChkpts) >= instance.f+1 {
			...
			if m := chkptSeqNumArray[len(chkptSeqNumArray)-(instance.f+1)]; m > H {
				...
				instance.outstandingReqBatches = make(map[string]*RequestBatch)
				logger.Infof("Replica %d application skipInProgress is true4", instance.id)
...
func (instance *pbftCore) witnessCheckpointWeakCert(chkpt *Checkpoint) {
	...
	//if instance.skipInProgress {
	if instance.seqNo < chkpt.SequenceNumber {
...
func (instance *pbftCore) recvCheckpoint(chkpt *Checkpoint) events.Event {
	...
	//if !instance.inW(chkpt.SequenceNumber) {//王剑修改，如果检查点不比当前的高度大或者当前高差距大于4000，不作检查点同步处理
	if !(chkpt.SequenceNumber > instance.h && chkpt.SequenceNumber-instance.h < instance.L*100) {
	...
	logger.Infof("Replica %d found %d matching checkpoints for seqNo %d, digest %s ,matching %d ,diffValues:%d",
		instance.id, matching, chkpt.SequenceNumber, chkpt.Id, matching, len(diffValues))
	...
	if count := len(diffValues); count > instance.f+1 {
		logger.Warningf("Network unable to find stable certificate for seqNo %d (%d different values observed already)",
	...
	if matching == instance.f+1 {
		...
		if ownChkptID, ok := instance.chkpts[chkpt.SequenceNumber]; ok {
			if ownChkptID != chkpt.Id {
				logger.Errorf("Own checkpoint for seqNo %d (%s) different from weak checkpoint certificate (%s)",
...
func (instance *pbftCore) unicast(msg *Message, receiverID uint64) error {
	msgRaw, err := proto.Marshal(msg)
	if err != nil {
		return fmt.Errorf("Cannot marshal message %s", err)
	}

	instance.consumer.unicast(msgRaw, uint64(receiverID))
...
func (instance *pbftCore) innerBroadcast(msg *Message) error {
	...
	if doByzantine {
		...
		for i := 0; i < instance.N*10; i++ { //王剑修改，最大允许N*10个通道，动态加节点机
...
func (instance *pbftCore) startSendCurrentTimer(timeout time.Duration, reason string) {
	logger.Debugf("Replica %d start peer and starting new view timer for %s: %s", instance.id, timeout, reason)
	instance.timerActive = true
	instance.newViewTimer.Reset(timeout, sendCurrentViewEvent{})
}
