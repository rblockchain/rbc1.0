...
func (vc *CurrentView) getSignature() []byte {
	return vc.Signature
}

func (vc *CurrentView) setSignature(sig []byte) {
	vc.Signature = sig
}

func (vc *CurrentView) getID() uint64 {
	return vc.ReplicaId
}

func (vc *CurrentView) setID(id uint64) {
	vc.ReplicaId = id
}

func (vc *CurrentView) serialize() ([]byte, error) {
	return pb.Marshal(vc)
}
