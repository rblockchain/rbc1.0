...
func (instance *pbftCore) correctViewChange(vc *ViewChange) bool {
	for _, p := range append(vc.Pset, vc.Qset...) {
		...
		if !(p.View < vc.View && p.SequenceNumber > vc.H) { //rongzer
	...
	for _, c := range vc.Cset {
		...
		if !(c.SequenceNumber >= vc.H) { //rongzer
...
func (instance *pbftCore) sendCurrentView() events.Event {
	...
	logger.Infof("Replica %d send CurrentView, v:%d, h:%d", instance.id, cv.View, cv.H)

	if instance.id != 0 {
		instance.unicast(&Message{Payload: &Message_CurrentView{CurrentView: cv}}, 0)
	} else {
		instance.innerBroadcast(&Message{Payload: &Message_CurrentView{CurrentView: cv}})
	}

	return nil
}

func (instance *pbftCore) recvCurrentView(cv *CurrentView) events.Event {
	logger.Infof("Replica %d received CurrentView from replica %d, v:%d, nv:%d, h:%d",
		instance.id, cv.ReplicaId, cv.View, instance.view, cv.H)
	rootCurrentH := instance.h / instance.K * instance.K //上一个检查点
	rootReceiveH := cv.H / instance.K * instance.K       //上一个检查点

	if rootCurrentH < rootReceiveH {
		instance.view = cv.View
		//发送sendViewChange
		instance.stopTimer()
		instance.startTimer(instance.oneSecondTimeout, "new view change")
		logger.Infof("Replica %d start time to sync view", instance.id)
		return nil
	}

	//判断vc的来源如果是根节点，则作部分特殊处理
	if cv.ReplicaId == 0 && instance.id != 0 {

		//视图不同步
		if instance.view < cv.View {
			instance.view = cv.View
		}
	...
	//根节点接收消息处理
	if cv.ReplicaId != 0 && instance.id == 0 {
		rootH := instance.h / instance.K * instance.K //上一个检查点

		if instance.view < cv.View {
			instance.view = cv.View

		} else if instance.view > cv.View || cv.H < rootH {
			instance.sendCurrentView()
		}
	}
...
func (instance *pbftCore) recvViewChange(vc *ViewChange) events.Event {
	//数据状态同步处理过程中，不进行viewchange处理
	if instance.consumer.IsStateInProgress() {
		return nil
	}

	logger.Infof("Replica %d received view-change from replica %d, v:%d, nv:%d, h:%d, |C|:%d, |P|:%d, |Q|:%d",
		instance.id, vc.ReplicaId, vc.View, instance.view, vc.H, len(vc.Cset), len(vc.Pset), len(vc.Qset))
	...
	if len(replicas) >= 1 { //rongzer 原来是f+1
	...
	if (!instance.activeView || instance.id == 0) && vc.View == instance.view && quorum >= instance.allCorrectReplicasQuorum() {
...
func (instance *pbftCore) sendNewView() events.Event {
	...
	if _, ok := instance.newViewStore[instance.view]; ok {
		logger.Warningf("Replica %d already has new view in store for view %d, skipping", instance.id, instance.view)
		return nil
	}
...
func (instance *pbftCore) processNewView() events.Event {
	...
	if !ok {
		logger.Warningf("Replica %d ignoring processNewView as it could not find view %d in its newViewStore", instance.id, instance.view)
		return nil
	}
	...
	outer:
		...
		if canExecuteToTarget {
			logger.Infof("Replica %d needs to process a new view, but can execute to the checkpoint seqNo %d, delaying processing of new view", instance.id, cp.SequenceNumber)
			return nil
		}
	...
	logger.Infof("Replica %d processNewView %d (%s), our most recent execution %d", instance.id, cp.SequenceNumber, cp.Id, speculativeLastExec)
...
func (instance *pbftCore) selectInitialCheckpoint(vset []*ViewChange) (checkpoint ViewChange_C, ok bool, replicas []uint64) {
	...
	for _, vc := range vset {
		for _, c := range vc.Cset { // TODO, verify that we strip duplicate checkpoints from this set
			...
			logger.Infof("Replica %d appending checkpoint from replica %d with seqNo=%d, h=%d, and checkpoint digest %s", instance.id, vc.ReplicaId, vc.H, c.SequenceNumber, c.Id)
		}
	}

	if len(checkpoints) == 0 {
		logger.Warningf("Replica %d has no checkpoints to select from: %d %s",
